...
import { drawRectangle, isInsideRectangle, isInResizeHandle, isInRotateHandle } from "./components/rectangle";
import { drawFreehand, isInsideFreehand } from "./components/freehand";
import { drawLine, isInsideLine, isInResizeHandleLine, isInRotateHandleLine } from "./components/line";
import { drawCircle, isInsideCircle, isInResizeHandleCircle } from "./components/circle";
  ...
  return {
    ...
    start(e) {
      ...
      if (this.tool === 'label') {
        ...
        if (text) {
          this.shapes.push({ type: 'label', x: pos.x, y: pos.y, text });  // Add label
    ...
    move(e) {
      ...
      if (this.isDragging && this.selectedIndex !== -1) {
        ...
        this.hoverInfo = {
          xDistance: Math.abs(shape.x - this.gardenRect.x) / this.scale,
          yDistance: Math.abs(shape.y - this.gardenRect.y) / this.scale,
        };
    ...
    redraw() {
      ...
      if (this.hoverInfo && this.selectedIndex !== -1 && this.isDragging) {
        ...
        const label = `${xDistance.toFixed(2)}m from left, ${yDistance.toFixed(2)}m from top`;
    ...
    getCursorStyle(pos) {
      ...
      const shape = this.findShapeAt(pos);
      return shape ? 'move' : this.tool === 'label' ? 'text' : 'crosshair';
    },
